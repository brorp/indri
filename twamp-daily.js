...
const OUTPUT_FILE = `output-twamp-daily-${output_name_str}.xlsx`;
const THRESHOLD_LT = 0.7;

// Only look at 1 day (last row before Grand Total)
const WINDOW_DAYS = 1;

const SHEET_DATA = ["DATA Twamp", "Twamp", "TWAMP", "DATA TWAMP", "DATA", "DATA ", "data"];
...
function decideStatus(lastVals) {
  // Need WINDOW_DAYS values AND ALL < 0.7
...
async function run(inputPath) {
  ...
  // --- 1) Build DATA map: CEK -> Map(ymd -> max(MAX TWAMP)) ---
  ...
  console.log(`📌 Rule: CLOSE only if last ${WINDOW_DAYS} day(s) ALL < ${THRESHOLD_LT}; missing => OPEN; no numeric => NO DATA`);
  console.log(`📌 Total CEK from SITE LIST: ${cekList.length}`);
}

const inputFile = process.argv[2];
if (!inputFile) {
  console.log('Usage: node twamp-daily.js "ANALISA TWAMP 23022026.xlsx"');
  process.exit(1);
}
