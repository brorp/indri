...
const OUTPUT_FILE = `output-s1-daily-${output_name_str}.xlsx`;
const THRESHOLD_GT = 99;

// Only look at 1 day (last day with valid data)
const WINDOW_DAYS = 1;

const SHEET_DATA = ["DATA", "DATA ", "DATA  ", "Sheet1"];
...
function decideStatus(lastVals) {
  // Need WINDOW_DAYS values AND ALL > 99
  if (!lastVals || lastVals.length < WINDOW_DAYS) return "open";
...
async function run(inputPath) {
  ...
  console.log(`📌 Rule: CLOSE only if each CEK's own last ${WINDOW_DAYS} day slot(s) are all numeric >= ${THRESHOLD_GT}; blank/<window => OPEN; no numeric => NO DATA`);
  console.log(`📌 Total CEK from SITE LIST: ${cekList.length}`);
}

const inputFile = process.argv[2];
if (!inputFile) {
  console.log('Usage: node s1-daily.js "ANALISA S1 SR 23022026 DAILY.xlsx"');
  process.exit(1);
}
